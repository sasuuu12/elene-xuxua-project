...
const Header: React.FC<HeaderProps> = ({ links }) => {
  ...
  return (
    <header className="bg-[#4A0E4E] text-white shadow-md relative z-50">
      ...
      {isOpen && (
        <div className="md:hidden bg-[#3A0B3D] absolute w-full left-0 z-50 border-t border-[#C3B1E1]/30 shadow-lg">
          <nav className="flex flex-col px-4 py-4">
            {links.map((link, index) => (
              <Link
                key={index}
                to={link.path}
                className="py-3 border-b border-white/10 hover:text-[#C3B1E1] transition-colors"
