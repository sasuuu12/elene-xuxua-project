...
```
src/
├── components/     → Header, Footer, Card, Badge, Button, SearchBar, ThemeToggle, ThemeContext
├── pages/          → About, Menu, Contact, NotFoundPage
├── layouts/        → MainLayout (Header + children + Footer)
├── hooks/          → usePageTracking (Custom Hook)
├── store/          → Redux Toolkit: CartSlice, Cart, hooks, index
├── types/          → TypeScript Interfaces (MenuItem, Navlink)
├── data/           → navLinks, menuItems, aboutContent
└── assets/         → WebP სურათები, ლოგო, ფონტი
```

---

## ✨ ბონუს: Redux Toolkit — სრული იმპლემენტაცია

კალათის (Cart) ფუნქციონალი სრულად Redux Toolkit-ით არის აგებული:

- **`configureStore`** — გლობალური store-ის კონფიგურაცია (`store/index.ts`)
- **`createSlice`** — 4 reducer: `addToCart`, `removeFromCart`, `decreaseQuantity`, `clearCart` (`store/CartSlice.ts`)
- **Typed Hooks** — `useAppDispatch` და `useAppSelector` (`store/hooks.ts`)
- **Cart UI** — კალათის კომპონენტი dispatch-ით და selector-ით (`store/Cart.tsx`)
- **გამოყენება** — მთავარი გვერდი, მენიუ, Header — 4 კომპონენტში

---

## 📊 Lighthouse ქულები

![Lighthouse შეფასება](<Screenshot 2026-04-28 085345.png>)
...
- WebP სურათები + `sharp` კომპრესია
- `font-display: swap` + ფონტის `preload`
- Vite `manualChunks` — vendor chunk splitting
- `loading="lazy"` + `decoding="async"` სურათებზე
- `React.memo(Card)` — მემოიზაცია
- `requestIdleCallback` — სკრიპტების defer